...
async def handle_webhook(queue: asyncio.Queue[str], request: web.Request):
    if request.headers.get("X-GitHub-Event") == "push":
        ...
        if json_data["repository"]["full_name"] == "kralverde/Pumpkin":
...
<html lang="en">
  ...
  <body>
      ...
      <p>This server is running <a href=https://github.com/Snowiiii/Pumpkin>Pumpkin MC</a> -- a Rust-written vanilla minecraft server -- straight from the master branch, updated every couple of hours (currently running <a href="https://github.com/Snowiiii/Pumpkin/commit/{{commit}}">{{name}} ({{short_commit}})</a>). You can also try to trigger a rebuild with <a href=https://pull.git.ci/process/kralverde/Pumpkin>this link</a> if you need to update the server! The server is running in debug mode, so it will be less efficient/slower than any release builds!</p>
